<template>
 <transition name="fade" mode="out-in">     
    <no-data v-if="isError" :isFull="isFull">
      ...
      <div class="slate-widget btn lgBtn" v-throttle @click="$emit('retry')">{{txtRetry}}</div>
    </no-data>
    <no-data v-else-if="isEmpty" :isFull="isFull">
      <slot name="empty">
        <p class="text-gray-800">{{txtNoData}}</p>
        <div v-if="!hideCreate" class="slate-widget btn lgBtn" v-throttle @click="$emit('create')">{{txtCreate}}</div>
      </slot>
    </no-data>
    <div v-else>
      <slot  />
      <div class="text-center font-pixie" v-if="!hidePageFooter">
        ...
        <div v-if="!hidePageNext">
          ...
          <div v-else class="slate-widget btn icoBtn mx-auto" v-throttle @click="$emit('nextPage')">
